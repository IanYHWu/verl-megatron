...
echo "Submitting job to Ray cluster at: ${head_node_ip}:${RAY_PORT}"

project_name='reasoning_cache'
exp_name='online_acemath_rl_30b_inst_hard_16k_no_summ_2_steps_2_samples'
...
python3 -c "import ray; print('Ray version:', ray.__version__); ray.init(); print('Connected nodes:', ray.nodes()); print('Resources:', ray.available_resources())"
echo "=================="
...
ray job submit --address="http://${head_node_ip}:${RAY_DASHBOARD_PORT}" \
  ...
  --runtime-env-json '{
    "env_vars": {
      "HF_HOME": "/project/flame/ianwu/huggingface/",
...
source /home/ianwu/anaconda3/bin/activate /project/flame/ianwu/conda/envs/verl_megatron
cd /home/ianwu/code/verl-megatron
...
python3 -m projects.reasoning_cache.main_ppo_reasoning_cache --config-name='ppo_megatron_trainer_rc' \
    ...
    trainer.rollout_data_dir=/tmp/ianwu/rollouts/${project_name}/${exp_name} \
